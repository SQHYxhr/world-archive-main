...
import { Edit3, Flag, Pin, Star } from "lucide-react";
import type { Entry, FactionProfile } from "@/types";
import { ENTRY_TYPE_LABELS, FACTION_CATEGORY_LABELS, FACTION_STATUS_LABELS } from "@/types";
...
export function FactionDetail({
  ...
  const profile: FactionProfile | undefined = entry.factionProfile;

  const parentFaction = useMemo(
    () =>
      profile?.parentFactionId
        ? projectEntries.find(
            (e) => e.id === profile.parentFactionId && e.type === "faction",
          )
        : undefined,
    [profile?.parentFactionId, projectEntries],
  );

  const headquartersLocation = useMemo(
    () =>
      profile?.headquartersLocationId
        ? projectEntries.find(
            (e) => e.id === profile.headquartersLocationId && e.type === "location",
          )
        : undefined,
    [profile?.headquartersLocationId, projectEntries],
  );
  ...
  const members = useMemo(
    () =>
      projectEntries.filter(
        (e) =>
          e.type === "character" &&
          e.characterProfile?.factionId === entry.id,
      ),
    [projectEntries, entry.id],
  );

  const governedLocations = useMemo(
    () =>
      projectEntries.filter(
        (e) =>
          e.type === "location" &&
          e.locationProfile?.governingFactionId === entry.id,
      ),
    [projectEntries, entry.id],
  );
    ...
    () =>
      projectEntries.filter(
        (e) =>
          e.type === "faction" &&
          e.id !== entry.id &&
          e.factionProfile?.parentFactionId === entry.id,
      ),
    [projectEntries, entry.id],
  );
  ...
  const hasProfile =
    !!profile &&
    (!!profile.factionCategory ||
      !!profile.status ||
      !!profile.parentFactionId ||
      !!profile.headquartersLocationId ||
      !!profile.ideology.trim() ||
      !!profile.structure.trim() ||
      !!profile.influence.trim() ||
      !!profile.history.trim() ||
      !!profile.creatorNotes.trim());

  const hasBasicInfo =
    !!profile?.factionCategory || !!profile?.status;

  const hasHierarchySection =
    !!profile?.parentFactionId || !!profile?.headquartersLocationId;

  const hasIdeologySection =
    (!!profile?.ideology.trim()) ||
    (!!profile?.structure.trim()) ||
    (!!profile?.influence.trim());
  ...
  const hasDerivedRelations = members.length > 0 || governedLocations.length > 0 || childFactions.length > 0;
        ...
        {entry.coverImage ? (
          ...
          <div className="border-b border-border/80 bg-gradient-to-b from-primary/5 to-transparent p-5">
            <div className="mb-2 flex flex-wrap items-center gap-2">
              <Badge variant="outline">组织档案</Badge>
              {entry.isPinned ? (
        ...
        <ScrollArea className="flex-1">
          <article className="space-y-6 p-5">
                  ...
                  <section className="rounded-xl border border-border/70 bg-card/40 p-4">
                    ...
                    <dl className="space-y-2">
                      {profile?.factionCategory ? (
                        <InfoRow
                          label="组织类型"
                          value={FACTION_CATEGORY_LABELS[profile.factionCategory] || profile.factionCategory}
                        />
                      ) : null}
                      {profile?.status ? (
                        <InfoRow
                          label="当前状态"
                          value={FACTION_STATUS_LABELS[profile.status] || profile.status}
                        />
                      ) : null}
                    </dl>
                  </section>
                ...
                {hasHierarchySection ? (
                  <section className="rounded-xl border border-border/70 bg-card/40 p-4">
                    ...
                    <dl className="space-y-2">
                      <InfoRow
                        label="上级组织"
                        value={parentFaction?.title ?? ""}
                        onClick={parentFaction ? () => onSelectEntry(parentFaction) : undefined}
                      />
                      <InfoRow
                        label="总部地点"
                        value={headquartersLocation?.title ?? ""}
                        onClick={headquartersLocation ? () => onSelectEntry(headquartersLocation) : undefined}
                      />
                    </dl>
                  </section>
                ) : null}
                ...
                {hasIdeologySection ? (
                  <section className="rounded-xl border border-border/70 bg-card/40 p-4">
                    ...
                    <div className="space-y-4">
                      <ProfileBlock title="理念 / 目标" text={profile?.ideology ?? ""} />
                      <ProfileBlock title="组织结构" text={profile?.structure ?? ""} />
                      <ProfileBlock title="影响力 / 势力范围" text={profile?.influence ?? ""} />
                    </div>
                  </section>
                ) : null}

                {/* 历史与备注 */}
                {hasHistorySection || hasCreatorNotes ? (
                  <section className="rounded-xl border border-border/70 bg-card/40 p-4">
                    <h3 className="mb-3 text-xs font-medium uppercase tracking-wider text-muted-foreground">
                      历史与备注
                    </h3>
              ...
              <section className="space-y-3">
                ...
                <dl className="space-y-2">
                  {members.length > 0 ? (
                    <InfoRow
                      label="成员角色"
                      value={members.map((c) => c.title).join("、")}
                    />
                  ) : null}
                  {governedLocations.length > 0 ? (
                    <InfoRow
                      label="管辖地点"
                      value={governedLocations.map((l) => l.title).join("、")}
                    />
                  ) : null}
                  {childFactions.length > 0 ? (
                    <InfoRow
                      label="下属组织"
                      value={childFactions.map((f) => f.title).join("、")}
                    />
                  ) : null}
                </dl>
                {/* Clickable relation cards */}
            ...
            <div className="border-t border-border/60 pt-4 text-xs text-muted-foreground">
              <p>
                {ENTRY_TYPE_LABELS.faction} · 创建于 {formatDate(entry.createdAt)}
              </p>
